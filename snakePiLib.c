...
int CompruebaColision(tipo_serpiente *serpiente, tipo_manzana *manzana, int comprueba_manzana) {
	...
	if (comprueba_manzana) {
		...
		// Compruebo si la cabeza de la serpiente rebasa los limites del area de juego y programamos la funcionalidad de jugar sin paredes
...
void InicializaJuego(fsm_t* this) {
	...
	tmr_startms((tmr_t*)(p_snakePi->tmr_refresco_snake), timer_decremento); //Inicializamos el Timer con periodo timer_decremento
...
void MueveSerpienteIzquierda (fsm_t* this) {
	...
	ActualizaSnakePi(p_snakePi);
		...
		if (CompruebaColision(&(p_snakePi->serpiente), &(p_snakePi->manzana), 0)){
			//printf("[FIN DEL JUEGO, PULSE LA TECLA 5 PARA VOLVER A EMPEZAR, 0 PARA SALIR]");
			piLock(SYSTEM_FLAGS_KEY);
...
void FinalJuego (fsm_t* this) {
	...
	pseudoWiringPiEnableDisplay(0); //PARA EL USO NORMAL DE LA TERMINAL, SIN EMULACIÓN DEL LED_DISPLAY
}
...
void ReseteaJuego (fsm_t* this) {
	...
	timer_decremento = 2000;	//Reseteamos el contador a su valor original
